...
def search(request):
	if request.GET:
		...
		query_type = 'artist'
		results = mbz_search(query, query_type)
		artists = []
		for artist in results['artist']:
			artists.append({'name':artist['name'], 'mbid':artist['id']})
		#return HttpResponse("Username found: %s" % username )
		return render_to_response('scrapers/home.html',{'page':'search', 'artists':artists},context_instance=RequestContext(request))
		# call username API
		# send to template that charts out each 
	else:
		#return HttpResponse("No username")
...
def mbz_search(query, type):
	url = "http://musicbrainz.org//ws/2/"+type+"/?query="+query+"&fmt=json"
	data = urllib2.urlopen(url)
	api_results = json.load(data)
	return api_results
